...
public final class CAN {
    ...
    /**
     * Motor and Steer value must be sent at the same time this method sends the old steer value
     * along with the new motor value
     *
     * @param motor value to be sent to VCU
    ...
    /**
     * Motor and Steer value must be sent at the same time this method sends the old motor value
     * along with the new steer value
     *
     * @param steer value to be sent to VCU
     * @throws InterruptedException from sendMotorAndSteerValue
     */
    ...
    /**
     * Runnable, launched by parent (CAN), that reads CAN packets into can frames by launching
     * candump and continuously parsing it's standard output into sensor frames that are put into
     * queues accessible by parent (CAN) object. Uses semaphores for mutex because the java keyword
    ...
    private class InputWorker implements Runnable {
         ...
         * @throws IOException throws if interrupted
         */
        ...
        private String readSensorLine() throws InterruptedException {
            ...
            while (!usSensorQueue.isEmpty()) {
                ...
                if (frame.data[0] == 0x10) {
                    ...
                    System.out
                            .println(String.format("Scrapping frame data: %s",
            ...
            while (true) {
                try {
                    ...
                    if (frame.identity.equals(CAN.VCU_ODOMETER_CAN_ID)) {
                        ...
                        System.out.println(
                                String.format("Unknown CAN frame: id=%s; data=%s", frame.identity,
                                        byteToHexString(frame.data)));
                    }
    ...
    /**
     * Runnable, launched by parent (CAN), that schedules sending of CAN frames put in queue by
     * parent When experimenting with the VCU we've found that it ignores commands if they are sent
     * too quickly. Uses semaphores for shared queues for the same reason as described above
     * InputWorker.
     */
